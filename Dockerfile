...
LABEL INSTALL-SYSEXT="/usr/bin/docker run --env IMAGE=IMAGE --env TARGET=/host/var/lib/extensions/gdm --rm --privileged --pid host -v /run:/run:rslave -v /etc/os-release:/etc/os-release:ro -v /:/host:shared -v /var/lib/containers:/var/lib/containers IMAGE  /bin/bash /systemd-sysext/import-from-oci.sh"
